...
zipでダウンロードされるはずなので、zipファイルを右クリックし、すべて展開を選択。  
展開したフォルダの中にある、`processing.exe`を開く。  
...
`;`は命令の区切りを表す重要な文字であるため、書き忘れるとエラーが出るため注意。
...
```java
void setup(){
  size(500,500);//定数や変数が使えないためやむなし
  background(255);
  fill(0);//テキストの色を黒に設定
  line(0,-O_Y+height,width,-O_Y+height);//x軸
  line(O_X,0,O_X,height);//y軸
  textAlign(LEFT,TOP);
  textSize(TEXT_SIZE);
  text("O",O_X,-O_Y+height);
  x=-O_X;
  py=0;
}
...
void draw(){
  float y=FUNC_A*x*x;
  line(x-1+O_X,-py-O_Y+height,x+O_X,-y-O_Y+height);//ひとつ前の座標(x-1,py)と(x,y)を線で結ぶ
  py=y;
  x++;
}
```
...
ヒント: 最後に操作されたキーの情報は`key`変数(宣言する必要はない)が持っている。
...
void draw(){
  ...
  line(x-1+O_X,-py-O_Y+height,x+O_X,-y-O_Y+height);
  py=y;
  x++;
}
```

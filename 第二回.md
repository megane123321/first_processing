...
問2. [第一回の問2](第一回.md#問2)のコードをループを用いて一度に描ききるように書き換えろ。
...
問3. 長さが10の配列を用意し、押されたキーが小文字のアルファベットである場合に入力された文字を配列の先頭から格納せよ。もし、配列がいっぱいになった場合、配列の中身を一字ずつ出力し、入力された文字を再び配列の先頭から格納せよ。  
